...
/**
 * Password change OTP verification screen
 * Displays 6-digit OTP input with auto-submit and resend functionality
 */
export default function VerifyPasswordOtpScreen() {
   ...
   const handleOtpComplete = useCallback(
      async (completeOtp: string) => {
         ...
         try {
            ...
            await verifyPasswordChangeOtp({ otp: completeOtp });
            ...
            if (err instanceof ApiError) {
               ...
               console.error('[VerifyPasswordOtp] Non-API error:', errorMessage);
   ...
   const handleResendOtp = useCallback(async () => {
      ...
      try {
         ...
         if (err instanceof ApiError) {
            ...
            const errorMessage =
               err instanceof Error ? err.message : 'Unknown error';
            console.error('[VerifyPasswordOtp] Resend OTP error:', errorMessage);
   ...
   // Redirect to account if user is not authenticated
   useEffect(() => {
      if (!user?.email) {
